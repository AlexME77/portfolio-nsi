# Portfolio – Travaux en Informatique (2024-2026)

Ce dépôt GitHub regroupe une sélection de mes travaux réalisés au lycée dans le cadre de la spécialité **Numérique et Sciences Informatiques (NSI)**.

L'objectif de ce dépôt est de présenter mon travail, mes projets et mes compétences aux établissements auxquels je candidate sur Parcoursup.

---

## Première

Travaux pratiques et exercices réalisés durant l'année de Première dans le cadre de la spécialité NSI.

Les énoncés des TP sont disponibles sur le [Site de mon professeur de NSI](https://mulot-nsi.fr/)

## Terminale

Chaque dossier correspond à un **chapitre ou une notion spécifique du programme** abordée en NSI (structures de données, bases de données, réseaux, etc.).

Les énoncés des TP sont disponibles à l'intérieur des dossiers.

---

## Technologies utilisées

Selon les projets, différents outils et langages sont utilisés :

* Python
* Algorithmique et structures de données
* Bases de données (SQLite, phpMyAdmin)
* Réseaux (Cisco Packet Tracer)
* Outils de versionnement : Git et GitHub

---

## Projet principal

### Projet robot GPS

Projet réalisé en Terminale dans le cadre de la spécialité NSI. Ce projet constitue un travail plus approfondi intégrant plusieurs notions du programme.

➡️ [Voir le projet sur GitHub](https://github.com/AlexME77/projet-nsi)
